...
public class Parser {
	...
	public Parser(Context c, List<Map<String, Object>> list) {
		context = c;
		returnList = list;
	}
	 ...
	 * moved "to right", and all "empty" will be equaled to transparent white {@link #EMPTY_COLOR} {@code = 0x00FFffFF = }{@value #EMPTY_COLOR}
	 * 
	 * @param ARGBhexes
	 ...
	 */
	private void hexesToSwatches(final String[] ARGBhexes,
			final int[] swatches, final int j) {
	...
	private String parseAuthor(final Node tmpNode) {
		final String author = tmpNode.getChildNodes().item(INDEX_AUTHOR_NODE)
				.getChildNodes().item(1).getFirstChild().getNodeValue();
		return author;
	}

	private String parseEditedAt(final Node tmpNode) {
		final String editedAt = tmpNode.getChildNodes().item(INDEX_EDITED_AT)
				.getFirstChild().getNodeValue();
		return editedAt;
	}

	private String parseID(final Node tmpNode) {
		final String ID = tmpNode.getChildNodes().item(INDEX_ID)
				.getFirstChild().getNodeValue();
		return ID;
	}

	private String parseName(final Node tmpNode) {
		final String themeName = tmpNode.getChildNodes().item(INDEX_TITLE)
				.getFirstChild().getNodeValue();
		return themeName;
	}
			...
			returnList.add(new HashMap<String, Object>() {
				{
					...
					put(__EDITED_AT, DateParser
							.fromKulerToHumanFormat(parseEditedAt(tmpNode)));
					int[] s = parseSwatches(tmpNode);
					put(__RATING, parseRating(tmpNode));
					put(__SWATCHES, s);
					put(_SMALL_PICTURE,
							ThemePicFactory.createNewPicFor(context, s));
					put(_BIG_PICTURE,
							ThemePicFactory.createBigImage(context, s, -1, -1));
				}
			});
		}
	}
	...
	private String parseRating(final Node tmpNode) {
		final String rating = tmpNode.getChildNodes().item(INDEX_RATING)
				.getFirstChild().getNodeValue();
		return rating;
	}
	...
	private int[] parseSwatches(final Node tmpNode) {
		...
		for (; j < 5; j++) {
			try {
				ARGBhexes[j] = "FF".concat(hex.getChildNodes().item(j)
